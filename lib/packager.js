...
Packager.prototype.isBroadcast = function(pkg, socket) {
  try {
    assert(this.node.peers.list.some(function(peer) { return peer.socket.remoteAddress == socket.remoteAddress }))
    assert(Packager.isValid(pkg))
...
/**
Checks if the passed $package is a HANDSHAKE:
{ type: "handshake"
, content: { remoteAddress: "0.0.0.0" }
}
*/
Packager.prototype.isHandshake = function(pkg, socket) {
  try {
    assert(typeof(pkg.type) ==  'string')
    assert(pkg.type == 'handshake')
    assert(typeof(pkg.content) == 'object')
    assert(net.isIP(pkg.content.remoteAddress))
  }catch(e) {
    return false
  }
  return true
