...
function Node(options) {
  ...
  this.peers.on('add', function(peer) {
    if(this.peers.list.length == 1) this.emit('connect')
    this.emit('new peer', peer)
  }.bind(this))

  this.peers.on('remove', function() {
    if(this.peers.list.length == 0) this.emit('disconnect')
  }.bind(this))
}
 ...
 * @param port
 */
Node.prototype.stop = function(port) {
  this.server.close()
  this.peers.close()
}
...
Node.prototype.requestPeers = function() {
  ...
  this.broadcast.sendPacket(['smoke', 'peer request'], {
    id: this.id
    , remoteAddress: this.remoteAddress
    , remotePort: this.options.port
    })
...
Node.prototype.onUnconnectedPeerRequest = function(socket, data) {
  ...
  this.broadcast.sendPacket(['smoke', 'peer request'], data)
