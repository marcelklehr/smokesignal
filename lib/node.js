...
Node.prototype.enterNetwork = function() {
  ...
  socket = node.openSocket(super_n.ip, super_n.port, function() {
    ...
    if(!node.opts.localIp || node.opts.localIp == '') {
      socket.write('{"type":"handshake","content":{"remoteAddress":"'+socket.remoteAddress+'"}}')
      return
    }
...
Node.prototype.ondata = function(socket, data) {
  ...
  try {
    ...
    try {
      var pkg = JSON.parse(data.split('}{')[0]+'}')
    }catch(e) {
  ...
  // HANDSHAKE //
  if(node.pkg.isHandshake(pkg, socket)) {
    logger.debug('type HANDSHAKE')
    if(!node.opts.localIp) {
      node.opts.localIp = pkg.content.remoteAddress
      if(!node.opts.isSuper) node.enterNetwork()
    }
    if(socket.writable) socket.write('{"type":"handshake","content":{"remoteAddress":"'+socket.remoteAddress+'"}}')
    socket.end()
  }else
