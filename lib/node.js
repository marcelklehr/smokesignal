...
function Node(options) {
  ...
  if(!this.remoteAddress) this.emit('error', new Error('Couldn\'t determine my own IP address'))
  ...
  this.peers.on('connect', function() {
    this.connected = true
  }.bind(this))
  this.peers.on('disconnect', function() {
    this.connected = false
  }.bind(this))
}
...
Node.prototype.start = function() {
  ...
  this.interval = setInterval(function() {
    ...
    if(this.options.entrance) {
      ...
      this.logger.debug('Sending ping to network entrance for detecting netsplits...')
      this.sendPing(this.options.entrance.address, this.options.entrance.port, function(er) {
        if(er) {
          this.logger.warn('Netsplit!')
          this.emit('disconnect')
          return this.enterNetwork()
        }
        this.logger.debug('Still connected to network entrance. Everything okay.')
      }.bind(this))
    }
